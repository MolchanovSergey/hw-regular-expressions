...
with open("phonebook_raw.csv", encoding="utf-8") as scvfile:
    rows = csv.reader(scvfile, delimiter=",")
...
def main(contacts_list: list):
    ...
    for item in contacts_list:
        delimiter = ' '
        full_name = delimiter.join(item[:3]).split(delimiter)
        result = [full_name[0], full_name[1], full_name[2], item[3], item[4],
            re.sub(PATTERN_PHONE, PATTERN_SUB, item[5]), item[6]]
